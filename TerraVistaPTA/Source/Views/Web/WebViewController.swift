...
class WebViewController: UIViewController {
   ...
   override func viewWillAppear(animated: Bool)
   {
      ...
      self.linkArray = WebLinkController.sharedInstance.getCKObjects() as! [WebLink]
      self.table.reloadData()
      self.table.hidden = false;
      
      WebLinkController.sharedInstance.storeLatestDate (NSDate(), usingKey: "PTAWebDate")
...
extension WebViewController : UITableViewDataSource
{
   ...
   func tableView(tableView: UITableView, moveRowAtIndexPath fromIndexPath: NSIndexPath, toIndexPath: NSIndexPath) {
      ...
      var pfObjArray = [CKRecord]()
      pfObjArray.append(itemToMove.rObj!)
      
      for (var i=toIndexPath.row+1; i < linkArray.count; i++)
      {
         let link = linkArray[i]
         link.order++
         pfObjArray.append(link.rObj!)
      }
      
    // Save all entries
    self.activityIndicator.startAnimating()
    WebLinkController.sharedInstance.updateAll { (error) -> Void in
        dispatch_async(dispatch_get_main_queue()) {
                if error == nil {
                    self.activityIndicator.stopAnimating()
