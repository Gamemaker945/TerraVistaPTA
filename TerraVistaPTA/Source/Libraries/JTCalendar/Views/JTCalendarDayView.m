...
- (void)commonInit
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(dateSelected:) name:@"kJTCalendarDaySelected" object:nil];
}

- (void) dateSelected:(NSNotification *) notification
{
    NSDictionary *data = notification.object;
    NSDate *selDate = [data objectForKey:@"SelectedDate"];
    if ([self.manager.dateHelper date:self.date isTheSameDayThan:selDate])
    {
        [self didTouch];
    }
}
